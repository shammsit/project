...
app.get("/signup", async (req, res) => {
  try {
    ...
    const filtered = sheetNames.filter(
      (name) =>
        name.toLowerCase() !== "other" && name.toLowerCase() !== "admin"
    );
...
// --- Admin control panel ---
app.get("/admin-control", requireAdminLogin, async (req, res) => {
  try {
    const authClient = await getAuthClient();
    const googleSheets = await getSheetsInstance(authClient);
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message:
        "Login and stay connected for the collaboration link. The link will be available here within 72 hours if you are eligible; otherwise, you will be contacted.",
    });
  } catch (error) {
        ...
        requests: [
          {
            deleteDimension: {
              range: {
                sheetId: 0, // adjust if "other" has different ID
                dimension: "ROWS",
...
app.post("/approve-user", requireAdminLogin, async (req, res) => {
  ...
  try {
    ...
    if (!rowRes.data.values || rowRes.data.values.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Row not found" });
    }
    ...
    res.json({
      success: true,
      message: `User approved & moved to sheet: ${projectName}`,
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message:
        "Failed to approve user. Ensure a sheet with the correct project name exists.",
    });
  }
});
